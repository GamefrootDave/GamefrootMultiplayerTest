Phaserfroot.PluginManager.register(
  ...
  class Test extends Phaserfroot.Component {
    constructor( target, instanceProperties ) {
      ...
      this.ping_timer = instanceProperties[ "ping timer" ];
      ...
      this.pingtextfield = ( typeof instanceProperties[ "pingtextfield" ] !== "undefined" ) ? this.scene.getChildById( instanceProperties[ "pingtextfield" ], true ) : null;
    ...
    onCreate () {
      ...
      this.pingtextfield = this.scene.addText( { x: 0, y: 0, textText: '200 ms' } );
      if ( !this.pingtextfield ) {
        this.reportError( "`Set Text Colour` block could not find an instance called [pingtextfield]." );
        return;
      }
      if ( !this.pingtextfield.setColor ) {
        this.reportError( "`Set Text Colour` block could not find text color information on an instance called [pingtextfield]." );
        return;
      }
      this.pingtextfield.setColor( "0x33ff33".replace( /^0x/, "#" ) );
      if ( !this.pingtextfield ) {
        this.reportError( "`Set Text Numeric` block could not find an instance called [pingtextfield]." );
        return;
      }
      if ( !this.pingtextfield.setFontSize ) {
        this.reportError( "`Set Text Numeric` block could not find text properties on an instance called [pingtextfield]." );
        return;
      }
      this.pingtextfield.setFontSize( 18 );
      if ( !this.pingtextfield ) {
        this.reportError( "`Set Instance X` block could not find the instance [pingtextfield]." );
        return;
      }
      this.pingtextfield.posX = 880;
      if ( !this.pingtextfield ) {
        this.reportError( "`Set Instance Y` block could not find the instance [pingtextfield]." );
        return;
      }
      this.pingtextfield.posY = 10;
      this.ping_timer = 0;
    }
    ...
    onTick () {
      ...
      this.scene.messageExternal( 'game-out', ['pingRequest', this.my_random_id] );
    }
    ...
    executeMessagegame_in () {
      ...
      // update ping
      if (this.value[0] == 'pingRequest') {
        if (this.value.slice(-1)[0] == this.my_random_id) {
          if ( !this.pingtextfield ) {
            this.reportError( "`Set Text on Text` block could not find an instance called [pingtextfield]." );
            return;
          }
          this.pingtextfield.components.getByName( "TextAutomation" )[ 0 ].text = (String(this.ping_timer) + 'ms');
          this.ping_timer = 0;
        }
      }
